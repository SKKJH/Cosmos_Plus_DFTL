...
GC_MGR::CheckAndStartGC(VOID)
{
	...
	IOUB_GLOBAL*	pstGlobal = IOUB_GLOBAL::GetInstance();
	...
	m_nVPC = pstGlobal->GetVPagePerVBlock() - IOUB_GLOBAL::GetVBInfoMgr(m_channel, m_way)->GetVBInfo(m_nVictimVBN)->GetInvalidLPNCount();
	...
	/*if (m_eIOType == IOTYPE_META) {
		IOUB_GLOBAL::GetInstance()->SetMetaGCing();
	}*/

	IOUB_IncreaseProfile(Prof_GC_count);
...
GC_MGR::IncreaseWriteCount(VOID)
{
	...
	IOUB_IncreaseProfile(Prof_GC_write);
...
GC_MGR::_Read(VOID)
{
	...
	IOUB_IncreaseProfile(Prof_GC_read);
...
GC_POLICY_GREEDY::Initialize(IOTYPE eIOType)
{
	...
	{
	case IOTYPE_GC:
		m_pstBlockMgr = IOUB_GLOBAL::GetUserBlockMgr();
		break;
...
GC_POLICY_GREEDY::GetVictimVBN(UINT32 channel, UINT32 way)
{
	...
	list_for_each_entry(VBINFO, pstVBInfo, &m_pstBlockMgr->m_dlUsedBlocks[channel][way][METATYPE_PAGE], m_dlList)
	{
	...
	IOUB_GLOBAL::GetInstance()->IncreaseProfileCount(PROFILE_BGC);
...
{
	INT32 nSize;
	nSize = IOUB_GLOBAL::GetInstance()->GetVPagePerVBlock() * sizeof(GC_DEBUG_FLAG);
