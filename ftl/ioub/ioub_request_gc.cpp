...
GC_REQUEST::_ProcessRead_Wait(VOID)
{
	...
	IOUB_GLOBAL::GetInstance()->IncreaseProfileCount(PROFILE_BGC_READ);
...
GC_REQUEST::_ProcessWrite_Wait(VOID)
{
	...
	pstActiveBlock = IOUB_GLOBAL::GetActiveBlockMgr(m_nChannel, m_nWay)->GetActiveBlock(m_eIOType);
	...
	if (bSuccess == TRUE)
	{
		...
		IOUB_GLOBAL::GetInstance()->IncreaseProfileCount(PROFILE_BGC_WRITE);
...
GC_REQUEST::_GetGCMgr(VOID)
{
	...
	{
		return IOUB_GLOBAL::GetGCMgr(m_nChannel, m_nWay);
	}
