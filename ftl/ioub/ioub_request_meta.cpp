...
VOID
META_REQUEST::Initialize(META_REQUEST_STATUS nStatus, UINT32 m_nMetaLPN, VOID* pstMetaEntry, IOTYPE eIOType, METATYPE eMetaType)
{
	UINT32 channel, way;
	channel = get_channel_from_lpn(m_nMetaLPN);
	way = get_way_from_lpn(m_nMetaLPN);
	m_nStatus		= nStatus;
	m_nMetaLPN		= m_nMetaLPN;
	m_pstMetaEntry	= pstMetaEntry;		// Meta Cache Entry
	m_bBufferHit	= FALSE;
	m_eIOType		= eIOType;
	m_nChannel = channel;
	m_nWay = way;
	m_metaType = eMetaType;
}
...
META_REQUEST::_ProcessRead_Done(VOID)
{
	...
	META_MGR*			pstMetaMgr = IOUB_GLOBAL::GetMetaMgr(m_metaType);
	...
	META_REQUEST_INFO*	pstRequestInfo = IOUB_GLOBAL::GetRequestMgr()->GetMetaRequestInfo();
...
META_REQUEST::_ProcessRead_Wait(VOID)
{
	...
	// GetPPN
	m_nVPPN = IOUB_GLOBAL::GetMetaL2VMgr(m_metaType)->GetL2V(nLPN);
	DEBUG_ASSERT(INVALID_PPN != m_nVPPN);

	// Check buffering 
	BUFFERING_LPN* pstBufferingLPN = IOUB_GLOBAL::GetMetaActiveBlockBufferingLPN(channel, way);
	...
	{
		// VNNAD Read
		IOUB_GLOBAL::GetVNandMgr()->ReadPage(stReqID, m_nVPPN, m_pstBufEntry->m_pMainBuf, m_pstBufEntry->m_pSpareBuf);
		bDone = FALSE;
	}
	...
	IOUB_GLOBAL::GetInstance()->IncreaseProfileCount(PROFILE_META_READ);
	*/
	return TRUE;
}
...
META_REQUEST::_ProcessWrite_Wait(VOID)
{
	...
	channel = get_channel_from_lpn(m_nMetaLPN);
	way = get_way_from_lpn(m_nMetaLPN);
	pstActiveBlock = IOUB_GLOBAL::GetActiveBlockMgr(channel, way)->GetActiveBlock(IOTYPE_META);
	...
	if (bSuccess == TRUE)
	{
		...
		IOUB_GLOBAL::GetInstance()->IncreaseProfileCount(PROFILE_META_WRITE);
	}
...
META_REQUEST_INFO::AllocateRequest(VOID)
{
	...
	if (IOUB_GLOBAL::GetBufferMgr()->IsEmpty() == TRUE)
	{
		return NULL;
	}
