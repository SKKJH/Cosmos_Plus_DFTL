...
#define DATA_SIZE_OF_BAD_BLOCK_TABLE_PER_DIE	(TOTAL_BLOCKS_PER_DIE)		// dy, 1byte per block
...
typedef struct _LOGICAL_SLICE_ENTRY {
	unsigned int virtualSliceAddr;					// Virtual address
} LOGICAL_SLICE_ENTRY, *P_LOGICAL_SLICE_ENTRY;

typedef struct _LOGICAL_SLICE_MAP {
	LOGICAL_SLICE_ENTRY logicalSlice[SLICES_PER_SSD];		// dy, L2P map
...
typedef struct _VIRTUAL_BLOCK_ENTRY {
	unsigned int bad : 1;
	unsigned int free : 1;
	unsigned int invalidSliceCnt : 16;			// dy, invalid LPN count
...
typedef struct _VIRTUAL_DIE_ENTRY {
	unsigned int currentBlock : 16;			// dy, active block number
...
typedef struct _BAD_BLOCK_TABLE_INFO_ENTRY{
	unsigned int phyBlock : 16;					// dy, PBN for bad block information, 0 in general (InitAddressMap())
...
typedef struct _PHY_BLOCK_ENTRY {
	...
	unsigned int bad :1;						// dy, 1: bad block
	unsigned int reserved0 :15;
