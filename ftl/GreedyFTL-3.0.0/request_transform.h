...
#define ROW_ADDR_DEPENDENCY_CHECK_OPT_SELECT	0			// operation start, read only
#define ROW_ADDR_DEPENDENCY_CHECK_OPT_RELEASE	1			// operation done, read only, prohibit erase while read

#define BUF_DEPENDENCY_REPORT_BLOCKED		0
#define BUF_DEPENDENCY_REPORT_PASS			1

#define ROW_ADDR_DEPENDENCY_REPORT_BLOCKED	0				// dependency check fail
...
typedef struct _ROW_ADDR_DEPENDENCY_ENTRY {
	unsigned int permittedProgPage : 12;				// dy, program order check purpose, program issue page no
	unsigned int blockedReadReqCnt : 16;				// dy, block ongoing read count
	unsigned int blockedEraseReqFlag : 1;				// dy, erased (1), 
