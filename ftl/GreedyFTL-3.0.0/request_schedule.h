...
typedef struct _DIE_STATE_ENTRY {
	unsigned int dieState	:	8;				// dy, DIE_STATE_IDLE, EXE, FAIL, IDLE -> EXE -> IDLE
	unsigned int reqStatusCheckOpt	:	4;		// dy, REQ_STATUS_CHECK_OPT_XXX, NONE, CHECK< REPORT, COMPLETION_FLAG,  NONE -> CHECK -> COMPLETION
	unsigned int prevWay	:	4;
	unsigned int nextWay 	:	4;				// dy, 다음 issue할 way number
...
typedef struct _WAY_PRIORITY_ENTRY {
	...
	unsigned int statusReportHead	:	4;		// dy, status update할 way 
	unsigned int statusReportTail 	:	4;
	unsigned int readTriggerHead	:	4;		// dy, read issue한 way
	unsigned int readTriggerTail	:	4;		// dy, read issue할 최근 way
	unsigned int writeHead	:	4;				// dy, write way number - old
	unsigned int writeTail	:	4;				// dy, write way number - new
	unsigned int readTransferHead	:	4;
	unsigned int readTransferTail	:	4;
	unsigned int eraseHead	:	4;
	unsigned int eraseTail	:	4;
	unsigned int statusCheckHead	:	4;		// dy, NAND status check list, old
	unsigned int statusCheckTail	:	4;		// dy, NAND status check list, new
	unsigned int reserved : 8;
} WAY_PRIORITY_ENTRY, *P_WAY_PRIORITY_ENTRY;

typedef struct _WAY_PRIORITY_TABLE {
	WAY_PRIORITY_ENTRY wayPriority[USER_CHANNELS];		// dy, read/write/erase 우선순위를 주기위해 사용하는 듯.
