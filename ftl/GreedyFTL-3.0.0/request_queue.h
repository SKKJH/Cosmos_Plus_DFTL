...
typedef struct _FREE_REQUEST_QUEUE
{
	...
	unsigned int reqCnt : 16;				// dy, free entry count
	unsigned int reserved0 : 16;
} FREE_REQUEST_QUEUE, *P_FREE_REQUEST_QUEUE;

typedef struct _SLICE_REQUEST_QUEUE
{
	unsigned int headReq : 16;				// dy, empty인 경우 REQ_SLOT_TAG_NONE
	unsigned int tailReq : 16;
...
typedef struct _BLOCKED_BY_BUFFER_DEPENDENCY_REQUEST_QUEUE
{
	unsigned int headReq : 16;
	unsigned int tailReq : 16;
	unsigned int reqCnt : 16;			// count of blocked
	unsigned int reserved0 : 16;
...
{
	unsigned int headReq : 16;						// dy, old request
	unsigned int tailReq : 16;						// dy, new request is at the tail
	unsigned int reqCnt : 16;
	unsigned int reserved0 : 16;
...
{
	unsigned int headReq : 16;			// dy, empty: REQ_SLOT_TAG_NONE
	unsigned int tailReq : 16;
	unsigned int reqCnt : 16;			// dy, die 별 request count
	unsigned int reserved0 : 16;
