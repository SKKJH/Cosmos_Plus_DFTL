...
char* DATA_BUFFER_MAP_ADDR;
char* DATA_BUFFER_BASE_ADDR;
char* COMPLETE_FLAG_TABLE_ADDR;
V2FMCRegisters	g_stV2FMC[USER_CHANNELS];


void InitFTL_win()
{
	unsigned int nSize = (unsigned int)RESERVED1_START_ADDR;
	DATA_BUFFER_MAP_ADDR = (char*)malloc(nSize);
	assert(DATA_BUFFER_MAP_ADDR);

	nSize = (unsigned int)(RESERVED_DATA_BUFFER_BASE_ADDR + RESERVED_DATA_BUFFER_SIZE);
	DATA_BUFFER_BASE_ADDR = (char*)malloc(nSize);
	assert(DATA_BUFFER_BASE_ADDR);

	nSize = (unsigned int)(TEMPORARY_PAY_LOAD_ADDR + (1 * 1024 * 1024));
	COMPLETE_FLAG_TABLE_ADDR = (char*)malloc(nSize);
	assert(COMPLETE_FLAG_TABLE_ADDR);
}
...
void CheckConfigRestriction()
{
...
#ifndef WIN32		// windows malloc does not guarantee below condition
	...
	if((unsigned int)FTL_MANAGEMENT_END_ADDR > DRAM_END_ADDR)
