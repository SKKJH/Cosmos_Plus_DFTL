...
#define AVAILABLE_DATA_BUFFER_ENTRY_COUNT				(16 * USER_DIES)		// 16 * 64 = 1024 buffer,
...
typedef struct _DATA_BUF_ENTRY {
	unsigned int logicalSliceAddr;			// dy, block number
	unsigned int prevEntry : 16;			// dy, for LRU 
	unsigned int nextEntry : 16;			// dy, for LRU
	unsigned int blockingReqTail : 16;
	unsigned int hashPrevEntry : 16;
	unsigned int hashNextEntry : 16;
	unsigned int dirty : 1;					// dy, DATA_BUF_DIRTY(1), DATA_BUF_CLEAN(0)
...
typedef struct _DATA_BUF_LRU_LIST {
	unsigned int headEntry : 16;				// dy, head에 최근 access된 것.
	unsigned int tailEntry : 16;				// dy, LRU
} DATA_BUF_LRU_LIST, *P_DATA_BUF_LRU_LIST;

typedef struct _DATA_BUF_HASH_ENTRY{
	unsigned int headEntry : 16;				// dy, for buffer cache hash
	unsigned int tailEntry : 16;				// dy ,for buffer cache hash
