...
unsigned int mbPerbadBlockSpace;				// bad block size (MB)
...
void InitAddressMap()
{
	...
	for(dieNo=0 ; dieNo<USER_DIES ; dieNo++)
	{
		for (blockNo = 0; blockNo < TOTAL_BLOCKS_PER_DIE; blockNo++)
		{
			...
			phyBlockMapPtr->phyBlock[dieNo][blockNo].bad = FALSE;		// dy
...
#ifdef WIN32
		// remove bad block
		*bbtTableChecker = BLOCK_STATE_NORMAL;
#endif
...
void InitBlockDieMap()
{
	...
	RecoverBadBlockTable((unsigned int)RESERVED_DATA_BUFFER_BASE_ADDR);

	//to prevent accessing bbtBlock by host
	// dy, bad block list가 저장된 block을 보존하기 위해 해당 block 인 PBN 0을 bad로 marking 해둔다.
	for(dieNo=0 ; dieNo<USER_DIES ; dieNo++)
...
{
	unsigned int virtualSliceAddr;		// dy, LPN
