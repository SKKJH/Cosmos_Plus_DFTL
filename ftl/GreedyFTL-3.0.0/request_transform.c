...
P_ROW_ADDR_DEPENDENCY_TABLE rowAddrDependencyTablePtr;			// dy, ?
...
// dy, nvme request를 slice로 나누어 sliceReqQ에 추가한다.
void ReqTransNvmeToSlice(unsigned int cmdSlotTag, unsigned int startLba, unsigned int nlb, unsigned int cmdCode)
{
	...
	if (cmdCode == IO_NVM_WRITE)
	{
		STAT_IncreaseCount(PROFILE_HOST_WRITE, loop);
		STAT_IncreaseCount(PROFILE_HOST_WRITE_REQ, 1);
	}
	else if (cmdCode == IO_NVM_READ)
	{
		STAT_IncreaseCount(PROFILE_HOST_READ, loop);
...
// dy, slice 정보를 이용하여 NAND request를 생성한다.
// dy, L2P translation 수행 
void DataReadFromNand(unsigned int originReqSlotTag)
{
	...
	{
		reqSlotTag = GetFromFreeReqQ();		// dy, allocate new request queue to IO NAND
		...
		reqPoolPtr->reqPool[reqSlotTag].nandInfo.virtualSliceAddr = virtualSliceAddr;			// dy, set PPN
...
// dy, slice 별로 NAND IO request를 생성하고 nandReqQ에 추가한다.
// dy, NAND request로 변환된 slice는 release 한다.
void ReqTransSliceToLowLevel()
{
	...
	while(sliceReqQ.headReq != REQ_SLOT_TAG_NONE)
	{
		...
		dataBufEntry = CheckDataBufHit(reqSlotTag);			// lookup buffer
		...
		else
		{
			...
			dataBufEntry = AllocateDataBuf();						// dy, buffer에 없으면 new buffer allocation
			reqPoolPtr->reqPool[reqSlotTag].dataBufInfo.entry = dataBufEntry;
		...
		if(reqPoolPtr->reqPool[reqSlotTag].reqCode  == REQ_CODE_WRITE)
		{
			...
			reqPoolPtr->reqPool[reqSlotTag].reqCode = REQ_CODE_RxDMA;			// dy, Receive data from host
		}
		else if(reqPoolPtr->reqPool[reqSlotTag].reqCode  == REQ_CODE_READ)
			reqPoolPtr->reqPool[reqSlotTag].reqCode = REQ_CODE_TxDMA;			// dy, transfer data to host
		else
			assert(!"[WARNING] Not supported reqCode. [WARNING]");

		reqPoolPtr->reqPool[reqSlotTag].reqType = REQ_TYPE_NVME_DMA;			// dy, set DMA type
		reqPoolPtr->reqPool[reqSlotTag].reqOpt.dataBufFormat = REQ_OPT_DATA_BUF_ENTRY;
...
// dy, 해당 address에 read/write/erase 가능한지 확인
//	read 중 erase 불가,
//	program 하지 않은 page read 불가.
//	program incremental하게 되는지.
unsigned int CheckRowAddrDep(unsigned int reqSlotTag, unsigned int checkRowAddrDepOpt)
{
	...
	if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_READ)
	{
		if(checkRowAddrDepOpt == ROW_ADDR_DEPENDENCY_CHECK_OPT_SELECT)
		{
			...
			rowAddrDependencyTablePtr->block[chNo][wayNo][blockNo].blockedReadReqCnt++;			// dy, block read count
		}
...
// dy, (reqType == REQ_TYPE_NVME_DMA) 
// dy, (reqType == REQ_TYPE_NANE) slice request로 부터 NAND request를 만들어 내고 nandReqQ에 추가.
void SelectLowLevelReqQ(unsigned int reqSlotTag)
{
	...
	if(bufDepCheckReport == BUF_DEPENDENCY_REPORT_PASS)
	{
		...
		else if(reqPoolPtr->reqPool[reqSlotTag].reqType  == REQ_TYPE_NAND)
		{
			...
			if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.rowAddrDependencyCheck == REQ_OPT_ROW_ADDR_DEPENDENCY_CHECK)
			{
				...
				PutToNandReqQ(reqSlotTag, chNo, wayNo);		// dy, nand request q에 추가.
			else
	...
	else if(bufDepCheckReport == BUF_DEPENDENCY_REPORT_BLOCKED)		// dy, blocked 인 경우에도 NAND read는 해야 valid 하게 된다.
	{
		if(reqPoolPtr->reqPool[reqSlotTag].reqType  == REQ_TYPE_NAND)		// dy, Process NAND IO 
			if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.rowAddrDependencyCheck == REQ_OPT_ROW_ADDR_DEPENDENCY_CHECK)
			{
...
void ReleaseBlockedByBufDepReq(unsigned int reqSlotTag)
{
	...
	if(reqPoolPtr->reqPool[reqSlotTag].nextBlockingReq != REQ_SLOT_TAG_NONE)
	{
		...
		reqPoolPtr->reqPool[targetReqSlotTag].prevBlockingReq = REQ_SLOT_TAG_NONE;			// set valid
...
// dy, HDMA request를 생성하여 controller HDMA Q SFR을 설정한다. (Tx / Rx 모두 처리함)
void IssueNvmeDmaReq(unsigned int reqSlotTag)
{
	...
	devAddr = GenerateDataBufAddr(reqSlotTag);		// dy, get DRAM buffer address
	numOfNvmeBlock = 0;

#if 0		// debugging
	if (reqSlotTag == 485)
	{
		xil_printf("ReqSlotID: 485\n");
	}
#endif
...
// dy, Check HDMA done and if done move to freeReqQ.
